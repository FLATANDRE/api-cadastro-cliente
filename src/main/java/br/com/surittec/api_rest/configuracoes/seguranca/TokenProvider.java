package br.com.surittec.api_rest.configuracoes.seguranca;
...
public class TokenProvider implements Serializable {
    ...
    public String createToken(String subject, List<String> authorities) throws JsonProcessingException {

        Date now = new Date();
        Date exp = new Date(System.currentTimeMillis() + EXPIRATION_TIME);

        String claim = authorities.stream()
            ...
            if (token.contains(TOKEN_PREFIX)) {  //Token JWT
                ...
                if (apiKey.isHabilitado()) {
                    if (apiKey.getDataVencimento().after(new Date())) {
                        ...
                        apiKey.getPerfis().forEach((p) -> {
                            authorities.add(new SimpleGrantedAuthority("ROLE_" + p.getCodigo().toUpperCase()));
                        });
